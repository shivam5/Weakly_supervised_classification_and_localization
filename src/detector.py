...
class Detector():
    ...
    def inference( self, rgb, train=False ):
        rgb *= 255.
        r, g, b = tf.split(value = rgb, num_or_size_splits = 3, axis = 3)
    ...
    def get_classmap(self, conv_input):
        # temp1 = tf.placeholder( tf.float32, [None, 14, 14, 1024], name="temp1")
        # temp1 = conv_input
        # conv6_resized = tf.squeeze(conv_input)
        conv6_resized = tf.image.resize_bilinear( conv_input, [self.image_height, self.image_width] )
        # print("The shape of label on input is ", tf.shape(label))
        # tf.Print(label, [label], message="This is label: ")
        with tf.variable_scope("GAP", reuse=True):
            label_w = tf.get_variable("W")
            # label_w = tf.gather(tf.transpose(tf.get_variable("W")), tf.cast(label, tf.int32)  )
            # label_w = tf.reshape( label_w, [-1, 1024, self.n_labels] ) # [batch_size, 1024, 1]
        # print("The shape of label now is ", tf.shape(label))
        # tf.Print(label_w, [label_w], message="This is label: ")

        conv6_resized = tf.reshape(conv6_resized, [self.image_height*self.image_width, 1024])

        classmap = tf.matmul( conv6_resized, label_w )
        classmap = tf.reshape( classmap, [self.image_height, self.image_width, self.n_labels] )
        # classmap = tf.reshape( classmap, [-1, self.image_height, self.image_width] )
        classmap = tf.transpose(classmap, perm=[2,0,1])
        return classmap
